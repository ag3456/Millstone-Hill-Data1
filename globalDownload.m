...
%                         30);
%
%  $Id: globalDownload.m 5819 2016-09-23 20:09:19Z brideout $
%
...
for i = 1:length(expArray)
    ...
    if (length(expName) > 0)
     ...
     if strcmp(format, 'simple')
        outputFile = strcat(outputDir, '/', basename, '.txt');
     elseif strcmp(format, 'hdf5')
        outputFile = strcat(outputDir, '/', basename, '.hdf5');
     elseif strcmp(format, 'netCDF4')
        outputFile = strcat(outputDir, '/', basename, '.nc');
     end
