%function [propertime, range, matrix, TID, n] = returnmatrix(filename, totype, varargin)
...
datainterested = data.ne; %index into struct to get data that you're interested in
matrix = zeros(length(range), length(time));
z = length(range);
l = 1;
...
% if (nargin == 6) %only if user specifies that a specific time and range is needed to be returned
%     d = varargin{3};
%     p = varargin{4};
%     fromrange = varargin{1};
%     torange = varargin{2};
% 
